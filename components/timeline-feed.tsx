...
// Separate component for tracking visibility with strict margins
const EventTracker: React.FC<{
  id: string
  onActiveChange?: (isActive: boolean) => void
  children: React.ReactNode
}> = ({ id, onActiveChange, children }) => {
  return (
    <motion.div
      id={`event-${id}`}
...
const EventCard: React.FC<EventCardProps> = ({ event, isVisible, onActiveChange }) => {
  ...
  return (
    <EventTracker id={event.id} onActiveChange={onActiveChange}>
      <motion.div
        className="flex flex-col gap-6"
        initial={{ opacity: 0, y: 30 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true, margin: "-100px" }} // Keep animation loose
